...
import { getCollisionDirection, getObjects } from "../../lib/CollisionHelpers.js";
...
export default class GameObject
{
	...
	update(dt) { }
	render(offset = { x: 0, y: 0 }, scale = { x: 1, y: 1 })
	{
	...
	onConsume(consumer) { this.wasConsumed = true; }
	onCollision(collider)
	{
		...
		if (this.isSolid)
		{
			switch (this.getEntityCollisionDirection(collider.hitbox))
			{
				case Direction.Up:
					collider.position.y = this.position.y - Math.abs(collider.position.y - collider.hitbox.position.y) - collider.hitbox.dimensions.y;
					break;
				case Direction.Down:
	 ...
	 */
	didCollideWithEntity(hitbox) { return this.hitbox.didCollide(hitbox) };
	...
	{
		let obj = getObjects(this, hitbox);
		return getCollisionDirection(obj.obj1, obj.obj2);
	}
}